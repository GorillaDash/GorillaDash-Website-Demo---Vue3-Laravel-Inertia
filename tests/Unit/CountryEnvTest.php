...
{
    $identity = [
        'APP_NAME' => "'Acme Diner ".strtoupper($country)."'",
...
function writeLayers(string $root, string $country, array $shared = [], array $local = []): void
{
    ...
    $local = [
        'APP_URL' => "http://acme-{$country}.test",
        'DB_DATABASE' => "acme_{$country}",
        ...$local,
    ];
...
it('keeps every deployment-only key out of the composed env', function () {
    ...
    expect($values)
        ...
        ->toHaveKey('APP_URL', 'http://acme-usa.test')
        ->toHaveKey('DB_DATABASE', 'acme_usa')
...
it('writes values verbatim so quoting and ${VAR} references survive', function () {
    ...
    expect($body)
        ->toContain("APP_NAME='Acme Diner USA'")
