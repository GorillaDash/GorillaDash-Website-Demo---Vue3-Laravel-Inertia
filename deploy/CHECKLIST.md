# Go-live checklist — Hungry Gorilla on GKE
...
| Artifact Registry repo | `hungry-gorilla`                                       |
| Namespace              | `hungry-gorilla-<country>` (e.g. `hungry-gorilla-usa`) |
...
- [ ] `build-and-push.sh` **auto-creates** this repo on first run. To create it up front instead:
  ```bash
  gcloud artifacts repositories create hungry-gorilla \
    --repository-format=docker --location=us-west1
  ```
- [ ] Images will be: `us-west1-docker.pkg.dev/gorilla-dash-178800/hungry-gorilla/{web,ssr}`
...
- [ ] Create the database (e.g. `hungry_gorilla_usa_k8s`) and a DB user
- [ ] Once per project: `make wi-gsa` (creates the `k8s-sql` GSA + grants `roles/cloudsql.client`)
- [ ] Per country: `make wi-bind COUNTRY=usa` (lets `hungry-gorilla-usa`'s KSA impersonate the GSA)
...
- [ ] `make reserve-ip COUNTRY=usa` — reserves `hungry-gorilla-usa-ip` if it isn't there yet and
      prints the address. Idempotent, so `make bootstrap` can re-run it.
- [ ] DNS **A record**: `hungry-gorilla-usa.example.com` → that IP
- [ ] In `deploy/k8s/overlays/usa/ingress.yaml`: set the domain (2 places) and uncomment
      `global-static-ip-name: hungry-gorilla-usa-ip`
...
- [ ] Confirm `overlays/<country>/ingress.yaml` still has its `spec.tls` block
      naming `hungry-gorilla-origin-tls` — the Secret on its own does nothing without it.
...
- [ ] `kubectl -n hungry-gorilla-usa get pods` — `hungry-gorilla`, `hungry-gorilla-scheduler`, `hungry-gorilla-worker` all Ready
...
- [ ] `hungry-gorilla-scheduler` / `hungry-gorilla-worker` pods Running (they idle until you
...
- **Service selector must be `role: web`.** A bare `app: hungry-gorilla` selector also
  ...
  Service selects `app: hungry-gorilla, role: web`.

- **ServiceAccount before the migrate Job.** The migrate Job uses
  `serviceAccountName: hungry-gorilla`, so `deploy.sh` applies the ServiceAccount before
...
| Read a secret back       | `kubectl -n hungry-gorilla-usa get secret hungry-gorilla-secret -o jsonpath='{.data.APP_KEY}' \| base64 -d`                                          |
| Read all secrets         | `kubectl -n hungry-gorilla-usa get secret hungry-gorilla-secret -o go-template='{{range $k,$v := .data}}{{$k}}={{$v\|base64decode}}{{"\n"}}{{end}}'` |
| Apply config-only change | edit `config.env` → `COUNTRY=usa ./deploy/deploy.sh` → `kubectl -n hungry-gorilla-usa rollout restart deploy`                                        |
| Tail web logs            | `kubectl -n hungry-gorilla-usa logs -f deploy/hungry-gorilla`                                                                                        |
| Tail SSR logs            | `kubectl -n hungry-gorilla-usa logs -f deploy/hungry-gorilla -c ssr`                                                                                 |
| Scale workers            | `kubectl -n hungry-gorilla-usa scale deploy/hungry-gorilla-worker --replicas=3`                                                                      |
| Run migrations only      | re-run `deploy.sh` (it runs the migrate Job), or apply `base/migrate-job.yaml` with the image substituted                                            |
| Open a shell in web      | `kubectl -n hungry-gorilla-usa exec -it deploy/hungry-gorilla -c web -- bash`                                                                        |
