# GKE deployment — Hungry Gorilla (Laravel + Inertia SSR, multi-country)
...
Per namespace there are also `hungry-gorilla-scheduler` (1 replica, `schedule:work`)
and `hungry-gorilla-worker` (`queue:work`, scale as needed).
...
└── overlays/
    ...
    │   ├── namespace.yaml       # the hungry-gorilla-usa Namespace (declarative)
...
DB creds) in the `hungry-gorilla-<c>` namespace.
...
- [ ] `gcloud artifacts repositories create hungry-gorilla --repository-format=docker --location=us-west1`
...
- [ ] Create database `hungryGorilla_usa` and a DB user.
...
- [ ] Point your DNS **A record** (`hungry-gorilla-usa.example.com`) at that IP.
- [ ] In `overlays/usa/ingress.yaml`: set the domain (2 places) and uncomment `global-static-ip-name: hungry-gorilla-usa-ip`.
...
- [ ] `kubectl -n hungry-gorilla-usa get pods` — web/ssr/scheduler/worker all Ready
...
- [ ] SSR check: `curl -s https://hungry-gorilla-usa.example.com | grep -o 'id="app">..*' | head -1` is non-empty
- [ ] Assets load over https (DevTools → Network: `/build/assets/*` are `https` 200)
- [ ] `hungry-gorilla-scheduler` / `hungry-gorilla-worker` Running (idle until you add scheduled tasks/jobs)
...
> `kubectl -n hungry-gorilla-<c> rollout restart deploy` so pods re-read it.
...
kubectl create namespace hungry-gorilla-nz && kubectl -n hungry-gorilla-nz create secret ...
...
```bash
# one key
kubectl -n hungry-gorilla-usa get secret hungry-gorilla-secret \
...
`kubectl -n hungry-gorilla-<c> rollout restart deploy` so pods pick it up.
...
`hungry-gorilla-origin-tls`. Without it every request through the edge answers **525**,
