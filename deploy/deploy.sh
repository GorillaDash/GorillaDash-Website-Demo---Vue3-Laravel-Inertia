...
#            REPO (hungry-gorilla), TAG (git short SHA), NAMESPACE (hungry-gorilla-<country>),
...
if ! kubectl -n "${NAMESPACE}" get secret hungry-gorilla-secret >/dev/null 2>&1; then
  echo "!! Secret 'hungry-gorilla-secret' not found in ${NAMESPACE}. Create it first (see README). Aborting."
  exit 1
fi

# 1) ConfigMap from this country's config.env first, so the migrate Job sees it.
kubectl -n "${NAMESPACE}" create configmap hungry-gorilla-config \
...
kubectl -n "${NAMESPACE}" delete job hungry-gorilla-migrate --ignore-not-found
sed "s#image: app-web#image: ${REG}/web:${TAG}#" "${ROOT}/deploy/k8s/base/migrate-job.yaml" \
  | kubectl -n "${NAMESPACE}" apply -f -
kubectl -n "${NAMESPACE}" wait --for=condition=complete --timeout=300s job/hungry-gorilla-migrate
...
kubectl -n "${NAMESPACE}" rollout status deployment/hungry-gorilla
kubectl -n "${NAMESPACE}" rollout status deployment/hungry-gorilla-scheduler
kubectl -n "${NAMESPACE}" rollout status deployment/hungry-gorilla-worker
