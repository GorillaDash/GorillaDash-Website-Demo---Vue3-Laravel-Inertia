# Deploy / ops shortcuts for Hungry Gorilla on GKE.
...
IP_NAME  ?= hungry-gorilla-$(COUNTRY)-ip
...
FROM        ?= usa
FROM_NS     ?= hungry-gorilla-$(FROM)
FROM_SECRET ?= hungry-gorilla-origin-tls
...
wi-bind: ## Bind this country's KSA to the GSA via Workload Identity (COUNTRY=<country>)
	gcloud iam service-accounts add-iam-policy-binding $(GSA) --project=$(PROJECT_ID) \
	  ...
	  --member="serviceAccount:$(PROJECT_ID).svc.id.goog[$(NS)/hungry-gorilla]"
...
origin-tls: ## Install an origin certificate as the Ingress TLS secret (CERT=... KEY=...)
	...
	@# certificate. Adding one also means referencing hungry-gorilla-origin-tls from that
	...
	$(KUBECTL) -n $(NS) create secret tls hungry-gorilla-origin-tls \
	  --cert="$(CERT)" --key="$(KEY)" --dry-run=client -o yaml | $(KUBECTL) apply -f -
	@echo ">> hungry-gorilla-origin-tls installed. Re-apply the Ingress if it was created before this: make deploy COUNTRY=$(COUNTRY)"
...
origin-tls-copy: ## Copy an origin cert into this namespace (FROM=usa COUNTRY=nz, or FROM_NS=<ns> FROM_SECRET=<name>) — only if it covers the TLS hosts
	...
	@$(KUBECTL) -n $(FROM_NS) get secret $(FROM_SECRET) -o json \
	  | jq '{apiVersion, kind, type, data, metadata: {name: "hungry-gorilla-origin-tls", namespace: "$(NS)"}}' \
	  | $(KUBECTL) apply -f -
...
bootstrap: ci-bind reserve-ip ns wi-bind apply-secret ## Per-country bring-up: everything automatable (COUNTRY=nz)
	...
	@echo "  CI deploy credentials, reserved IP, namespace $(NS), Workload Identity binding, hungry-gorilla-secret"
	@echo ""
...
status: ## Deploys + ingress + cert status
	...
	@$(KUBECTL) -n $(NS) describe managedcertificate hungry-gorilla-cert 2>/dev/null | grep -E "Status|Domains" || true

logs-web: ## Tail web logs
	$(KUBECTL) -n $(NS) logs -f deploy/hungry-gorilla -c web

logs-ssr: ## Tail SSR logs
	$(KUBECTL) -n $(NS) logs -f deploy/hungry-gorilla -c ssr

logs-worker: ## Tail queue worker logs
	$(KUBECTL) -n $(NS) logs -f deploy/hungry-gorilla-worker

logs-scheduler: ## Tail scheduler logs
	$(KUBECTL) -n $(NS) logs -f deploy/hungry-gorilla-scheduler

restart: ## Restart all deploys (pick up config.env changes)
	$(KUBECTL) -n $(NS) rollout restart deploy

scale-worker: ## Scale queue workers (REPLICAS=3)
	$(KUBECTL) -n $(NS) scale deploy/hungry-gorilla-worker --replicas=$(REPLICAS)

migrate: ## Run DB migrations in a running web pod
	$(KUBECTL) -n $(NS) exec deploy/hungry-gorilla -c web -- php artisan migrate --force

shell: ## Shell into the web container
	$(KUBECTL) -n $(NS) exec -it deploy/hungry-gorilla -c web -- bash

secret: ## Print the decoded Secret
	$(KUBECTL) -n $(NS) get secret hungry-gorilla-secret \
